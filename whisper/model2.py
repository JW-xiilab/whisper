...
from .decoding2 import decode as decode_function
from .decoding2 import detect_language as detect_language_function
from .transcribe2 import transcribe as transcribe_function_kjw
...
class MultiHeadAttention(nn.Module):
    layer_count = 0
    def __init__(self, n_ctx, n_state: int, n_head: int):
        super().__init__()
        self.n_head = n_head
    ...
    def forward(
        ...
        if kv_cache is not None and k.shape[1] <= self.n_ctx:
            ...
            # medium: 24
            # large: 32
            key_id = self.layer_id - (32)
            value_id = key_id + 1
            size = k.shape[1]
            kv_cache[key_id, :, offset:offset+size, :] = k
            kv_cache[value_id, :, offset:offset+size, :] = v
            k = kv_cache[key_id][:,:offset+size,:].to(x.device).to(x.dtype)
            v = kv_cache[value_id][:,:offset+size,:].to(x.device).to(x.dtype)
...
class Whisper(nn.Module):
    def __init__(self, dims: ModelDimensions, name):
        super().__init__()
        self.type = name if name else None
        self.dims = dims
    ...
    def new_kv_cache(self, n_group: int, length: int):
        ...
        elif self.type == "large" or self.type == "large-v2":
